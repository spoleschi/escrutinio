...
def exportar_excel():
    try:
        ...
        df['Participación'] = (df['TotalVotos'] / df['CantidadVotantes'] * 100).fillna(0).round(2).astype(str) + '%'
...
def login():
    try:
        ...
        # Hash de la contraseña (si las contraseñas están hasheadas en la base de datos)
...
def get_datos():
    try:
        ...
        # Consulta para obtener datos combinados con la nueva estructura
        query = """
        SELECT 
            s.Sucursal,
            s.Cantidad as CantidadVotantes,
            CASE 
                WHEN e.Cod_Ubic IS NOT NULL THEN 1 
                ELSE 0 
            END as Procesado,
            COALESCE(e.Lista_Blanca, 0) as Lista_Blanca,
            COALESCE(e.Lista_Celeste, 0) as Lista_Celeste,
        ...
        totales = {
            'Lista_Blanca': int(df['Lista_Blanca'].sum()),
            'Lista_Celeste': int(df['Lista_Celeste'].sum()),
            'En_Blanco': int(df['En_Blanco'].sum()),
            'Anulados': int(df['Anulados'].sum()),
            'Recurridos': int(df['Recurridos'].sum()),
            'Observados': int(df['Observados'].sum()),
...
def borrar_sucursal(sucursal):
    ...
    try:
        ...
        # Borrar el registro
        query = "DELETE FROM Elecciones2025 WHERE Cod_Ubic = ?"
